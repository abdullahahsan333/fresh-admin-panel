            ...
            <div class="flex justify-center items-center">
                <div class="w-full">
                    <form method="POST" action="{{ route('login') }}" class="bg-white shadow rounded p-6 w-full max-w-sm">
                        @csrf
                        <h1 class="text-xl font-semibold mb-4">User Login</h1>
                        ...
                        <p class="text-xs text-gray-500 mt-4">No account? <a href="{{ route('register') }}" class="underline text-[rgb(var(--color-primary))]">Register</a></p>
                    </form>
                </div>
            </div>
        </div>
