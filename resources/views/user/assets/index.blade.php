...
<form id="assetsForm" action="{{ route('user.assets.store') }}" method="POST" data-server-details-url="{{ route('user.assets.server.details') }}" data-server-save-url="{{ url('/user/server_store') }}" data-server-route-base="{{ url('/user/server') }}">
    @csrf
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                {{-- <form action="{{ route('user.import') }}" method="POST">
                    @csrf
                    <button type="submit" class="btn btn-secondary">Import from Admin</button>
                </form> --}}
    ...
    <div class="grid grid-cols-2 lg:grid-cols-2 gap-6 mt-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mt-4">
                <div class="text-sm text-gray-500">Add your server IP to configure services.</div>
                <div class="mt-3 flex gap-3">
                    <input id="assetInput" type="text" class="flex-1 border border-gray-300 rounded-lg px-3 h-10 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[rgb(var(--color-primary))]" placeholder="Add IP like 128.199.73.128">
                    <button id="assetAddBtn" type="button" class="btn btn-primary h-10 px-4">Add</button>
                </div>
            </div>
    ...
    <div class="grid grid-cols-1 gap-6 mt-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="mt-6">
                ...
                <div id="servicesSelectContainer" class="mt-3 flex flex-wrap gap-3"></div>
