<header class="h-12 lg:h-16 bg-white border-b border-gray-200 flex items-center justify-between px-2 sm:px-4 ">
    <div class="flex items-center gap-2">
        <button id="sidebarToggle" class="inline-flex items-center justify-center h-9 w-9 lg:h-10 lg:w-10 rounded-lg hover:bg-gray-100" title="Toggle sidebar" aria-pressed="false">
    ...
    <div class="flex items-center gap-4">
        <button id="topbarNotifBtn" class="relative h-9 w-9 lg:h-10 lg:w-10 grid place-items-center rounded-lg hover:bg-gray-100">
        ...
        <button id="topbarProfileBtn" class="h-9 w-9 lg:h-10 lg:w-10 rounded-full overflow-hidden hover:ring-2 ring-gray-200">
            <img class="h-full w-full rounded-full object-cover" src="{{ ($adminUser && !empty($adminUser->avatar)) ? asset($adminUser->avatar) : 'https://i.pravatar.cc/80?img=5' }}" alt="">
        </button>
    </div>
</header>
