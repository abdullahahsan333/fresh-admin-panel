...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const topbarProfileBtn = document.getElementById('userTopbarProfileBtn');
    const topbarDropdownId = 'topbarProfileDropdown';
    let topbarDropdown = document.getElementById(topbarDropdownId);
    const topbarNotifBtn = document.getElementById('userTopbarNotifBtn');
    ...
    const mobileSidebarEl = document.getElementById('mobileUserSidebar');
    const mobileSidebarCloseBtn = document.getElementById('mobileUserSidebarCloseBtn') || document.getElementById('mobileSidebarCloseBtn');
    let menuFlyout = null;
    let subFlyout = null;
    const flyouts = {};
        ...
        if (!profileDropdown) {
            ...
            profileDropdown.innerHTML = `
                ...
                <nav class="p-2 text-sm">
                    <a href="/user/profile" class="flex items-center gap-2 px-2 py-2 rounded hover:bg-gray-50">
                    ...
                    <a href="/user/assets" class="flex items-center gap-2 px-2 py-2 rounded hover:bg-gray-50">
                    ...
                    <a href="/user/logout" class="flex items-center gap-2 px-2 py-2 rounded hover:bg-gray-50">
    ...
    function setupSidebarMenus(sidebarElement) {
        const sidebar = sidebarElement || document.getElementById('userSidebar');
        ...
        sidebar.addEventListener('click', (e) => {
            ...
            if (collapsed) {
                ...
                if (willExpand) { openFlyoutForPanel(panel, rect, level); }
            } else {
    ...
    if (profileBtn || mobileProfileBtn) {
        ...
        document.addEventListener('click', (e) => {
            const t = e.target;
            const inSidebar = t.closest('#userSidebar');
            const inFlyout = t.closest('[id^="sidebarFlyout"]');
            const isMenuTrigger = t.closest('[data-menu-toggle]') || t.closest('[data-submenu-toggle]');
            const isProfileTrigger = t.closest('#sidebarProfileBtn') || t.closest('#mobileSidebarProfileBtn') || t.closest('#userTopbarProfileBtn') || t.closest('#userTopbarNotifBtn');
            ...
            topbarNotifDropdown.innerHTML = `
                ...
                <div class="max-h-80 overflow-auto">
                    <a href="javascript:void(0);" class="flex items-start gap-3 px-3 py-3 hover:bg-gray-50">
                        ...
                        <div class="text-sm"><div class="font-medium text-gray-800">Welcome to your dashboard</div><div class="text-xs text-gray-500">Just now</div></div>
                    </a>
                </div>
                <div class="p-3 border-t border-gray-100">
                    <a href="/user/dashboard" class="inline-flex items-center justify-center w-full h-9 rounded-lg bg-[rgb(var(--color-primary))] text-white text-sm">Show all</a>
                </div>
            `;
    ...
    const setCurrentIp = (ip) => {
        ...
        const detailsUrl = assetsForm?.getAttribute('data-server-details-url') || '/user/assets/server-details';
    ...
    const addServerIp = () => {
        ...
        const token = document.querySelector('#assetsForm input[name="_token"]')?.value || '';
        const saveUrl = assetsForm?.getAttribute('data-server-save-url') || '/user/assets/server';
